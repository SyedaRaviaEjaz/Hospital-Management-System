...
int login(char a){
	...
	static int count;
    	ch1 = a;
    	string userid[3] = {"2054645", "2054654", "2043534"};
		string drname = "name1";
    	string id;
    	bool flag = true;
        while(flag){
...
class doctor;      //forward declaration of class doctor
class admin:virtual public ward{
public:
	int in;
	float ward_charges;
    float test_charges;
...
class doctor: public department, virtual public ward, public patient{
...
public:
	...
	void depmenu(){
		...
		drfile.open("Appointments (1).txt", ios::in );
		...
		void searchPatientRecords() { 	
			...
			cout<<"\n\n\t\t-> Do you want to view more patient records?(Y/N): ";
			cin>>response;
			response = toupper(response);
		...
		void delPatientRecords(){
			...
			cout<<"\n\n\t\t\t-> Enter account's password to proceed with deletion\n";
			ch = _getch();
            while(ch != 13){
            pass.push_back(ch);
            cout << '*';
			...
			file3.open( patientfile.c_str() , ios::out);
			if(!file3){
			...
			file3<<"No records exist for patient ID: "<<patientfile<<" The contents of this file have been deleted."; 
		...
		void prescribeMeds() { 
			...
			cout<<"\n\t\t-> Do you want to add any other treatment information?(Y/N) \n";
			cin>>response;
			response = toupper(response);
			
			switch(response){
				case 'Y':{
					cout<<"\t\t\t-> Enter additional treatment information : ";cin.ignore();gets(p.extraTreat);
	...
	void salary(){
        ...
        case 'Y' :{
        	char response;
			cout<<"\n\t\t\t-> Enter the total hrs worked overtime: ";
            cin>>hrs;
            salary_amount = salary_amount + (7 - hrs)*hr_rate;
            cout<<"\t\t\t   -------------------------------\n";
            cout<<"\t\t\t-> Total salary for this month is: Rs"<<salary_amount<<"/-"<<endl;
            cout<<"\n\n\t\t\t->Go back to menu?(Y/N): ";
        	cin>>response;
		    response = toupper(response);
...
public:
	...
	~bill(){          //destructor to free dynamically allocated memory
		delete amount;
	}
};
...
public:
	...
	if(!file){
		cout<<"\n\n\n\t\t\t-> The record of patient does not exist";		
	}
	else{
    cout<<"\n\n\t\t   <><><><><><><><><><><><><><><><><><><><>\n\n";
    cout<<"\t\t\t--------------------------\n";
                ...
                while (flag){
                    ...
                    switch (c){
                        case 1 :{
                            s.add_staff();
                            system("CLS");
                            cout << "\n\n\n\n\t\t\t\tIf you want to go back to the menu, enter ' y ': ";
                            char f;
                            cin>>f;
                            if(f!='y'){
                            flag = false;
